...
const scanMam=async()=>{if(!confirm(`Run a MAM scan for un-scanned books across ${sel.size} author(s)? This may take a while.`))return;setScanning(true);try{const r=await api.post("/authors/scan-mam",{author_ids:[...sel]});if(r.error){toast.error(r.error)}else{toast.success(`MAM scan complete: ${r.scanned||0} scanned, ${r.found||0} found, ${r.possible||0} possible, ${r.not_found||0} not on MAM`);setSel(new Set());setSelMode(false);reload()}}catch(e){toast.error(e.message||"MAM scan failed")}setScanning(false)};
