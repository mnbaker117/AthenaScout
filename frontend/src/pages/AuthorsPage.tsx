...
export default function AuthorsPage({onNav}){const t=useTheme();const[aus,setAus]=useState([]);const[ld,setLd]=useState(true);const[q,setQ]=usePersist("ap_q","");const[sort,setSort]=usePersist("ap_sort","name");const[vm,setVm]=usePersist("ap_vm","list");
const[selMode,setSelMode]=useState(false);const[sel,setSel]=useState(new Set());const[clearing,setClearing]=useState(false);const[scanning,setScanning]=useState(false);const[mamOn,setMamOn]=useState(false);
...
const scanSources=async()=>{if(!confirm(`Run a source-plugin lookup for ${sel.size} author(s)? This may take a while.`))return;setScanning(true);try{const r=await api.post("/authors/scan-sources",{author_ids:[...sel]});toast.info(`Source scan started — ${r.total||sel.size} authors. Track progress on the Dashboard.`);setSel(new Set());setSelMode(false);window.dispatchEvent(new CustomEvent("athenascout:scan-started"))}catch(e){toast.error(e.message||"Scan failed to start")}setScanning(false)};
const scanMam=async()=>{if(!confirm(`Run a MAM scan for un-scanned books across ${sel.size} author(s)? This may take a while.`))return;setScanning(true);try{const r=await api.post("/authors/scan-mam",{author_ids:[...sel]});if(r.error){toast.error(r.error)}else if(r.status==="complete"){toast.info(r.message||"No un-scanned books")}else{toast.info(`MAM scan started — ${r.total||0} books. Track progress on the Dashboard.`);window.dispatchEvent(new CustomEvent("athenascout:scan-started"))};setSel(new Set());setSelMode(false)}catch(e){toast.error(e.message||"MAM scan failed")}setScanning(false)};
useEffect(()=>{const c=new AbortController();setLd(true);api.get(`/authors?search=${encodeURIComponent(q)}&sort=${sort}`,c.signal).then(d=>{setAus(d.authors||[]);setLd(false)}).catch(e=>{if(!api.isAbort(e))setLd(false)});return()=>c.abort()},[q,sort]);
const AuthorCard=({a})=><div onClick={()=>selMode?toggleSel(a.id):onNav("author",a.id)} style={{minWidth:150,maxWidth:180,flex:"1 1 150px",background:selMode&&sel.has(a.id)?t.accent+"15":t.bg2,border:`1px solid ${selMode&&sel.has(a.id)?t.accent:t.borderL}`,borderRadius:10,padding:16,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:8,textAlign:"center",transition:"background 0.15s, border-color 0.15s"}}>{a.image_url?<img src={a.image_url} alt="" style={{width:64,height:64,borderRadius:"50%",objectFit:"cover"}}/>:<div style={{width:64,height:64,borderRadius:"50%",background:t.bg4,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,fontWeight:700,color:t.tg}}>{a.name?.charAt(0)}</div>}<div style={{fontSize:13,fontWeight:600,color:t.text2,display:"flex",alignItems:"center",gap:4,justifyContent:"center"}}>{a.name}{(a.link_count||0)>0?<span title={`${a.link_count} linked`} style={{display:"inline-flex",padding:"0 4px",borderRadius:3,fontSize:9,fontWeight:500,background:t.purb||t.bg4,color:t.purt,border:`1px solid ${t.pur}33`}}>↔{a.link_count}</span>:null}</div><div style={{display:"flex",gap:8,fontSize:11}}><span style={{color:t.grnt}}>{a.owned_count||0}</span><span style={{color:t.tg}}>/</span><span style={{color:t.ylwt}}>{a.missing_count||0}</span></div><div style={{width:"100%"}}><PB owned={a.owned_count||0} total={a.total_books||0}/></div></div>;
const AuthorRow=({a})=><div onClick={()=>selMode?toggleSel(a.id):onNav("author",a.id)} style={{display:"flex",alignItems:"center",gap:14,padding:"10px 14px",borderRadius:8,cursor:"pointer",background:selMode&&sel.has(a.id)?t.accent+"15":t.bg2,border:`1px solid ${selMode&&sel.has(a.id)?t.accent:t.borderL}`,transition:"background 0.15s, border-color 0.15s"}}>{a.image_url?<img src={a.image_url} alt="" style={{width:40,height:40,borderRadius:"50%",objectFit:"cover"}}/>:<div style={{width:40,height:40,borderRadius:"50%",background:t.bg4,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:700,color:t.tg}}>{a.name?.charAt(0)}</div>}<div style={{flex:1,minWidth:0}}><div style={{fontSize:14,fontWeight:600,color:t.text2,display:"flex",alignItems:"center",gap:6}}>{a.name}{(a.link_count||0)>0?<span title={`${a.link_count} linked author${a.link_count>1?"s":""}`} style={{display:"inline-flex",alignItems:"center",gap:3,padding:"1px 6px",borderRadius:4,fontSize:10,fontWeight:500,background:t.purb||t.bg4,color:t.purt,border:`1px solid ${t.pur}33`}}>↔ {a.link_count}</span>:null}</div><div style={{display:"flex",gap:12,fontSize:12,marginTop:2}}><span style={{color:t.grnt}}>{a.owned_count||0} owned</span><span style={{color:t.ylwt}}>{a.missing_count||0} missing</span><span style={{color:t.purt}}>{a.series_count||0} series</span></div></div><div style={{width:80}}><PB owned={a.owned_count||0} total={a.total_books||0}/></div></div>;
