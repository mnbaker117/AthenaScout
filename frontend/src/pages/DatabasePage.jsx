...
const deleteRow=async(rowId)=>{try{await api.del(`/db/table/${tab}/row/${rowId}`);setDeleting(null);load();setSaveMsg({type:"success",count:0,msg:`Row ${rowId} deleted`});setTimeout(()=>setSaveMsg(null),3000)}catch(e){setDeleting(null);setSaveMsg({type:"error",errors:[{error:e.message||String(e)}]})}};
