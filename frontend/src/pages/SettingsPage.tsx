...
const[mamVld,setMamVld]=useState(false);const[mamRes,setMamRes]=useState<any>(null);const[dragIdx,setDragIdx]=useState<number|null>(null);const[testRun,setTestRun]=useState(false);const[testRes,setTestRes]=useState<any>(null);const[newSrcPath,setNewSrcPath]=useState("");const[newSrcType,setNewSrcType]=useState("root");const[newSrcApp,setNewSrcApp]=useState("calibre");const[pathVld,setPathVld]=useState(false);const[pathRes,setPathRes]=useState<any>(null);useEffect(()=>{api.get("/settings").then(setS).catch(console.error);api.get("/version").then(r=>setBuildSha(r.short_sha||"")).catch(()=>{})},[]);
...
<SF label="Hermeece API Key" desc="Shared token for the Send-to-Hermeece endpoint. Generate it in Hermeece → Credentials → AthenaScout shared API key, then paste the value here.">{s.hermeece_api_key_set&&!s._editingHKey?<div style={{display:"flex",alignItems:"center",gap:12}}><span style={{fontSize:14,color:t.tm,letterSpacing:"3px"}}>••••••••</span><Btn size="sm" onClick={()=>upd("_editingHKey",true)}>Change</Btn></div>:<div style={{display:"flex",flexDirection:"column",gap:6}}><input value={s._editingHKey?s._newHKey||"":s.hermeece_api_key||""} onChange={e=>s._editingHKey?upd("_newHKey",e.target.value):upd("hermeece_api_key",e.target.value)} placeholder="64-char hex token" style={{...ist,width:220,fontFamily:"ui-monospace, SFMono-Regular, Consolas, monospace"}}/>{s._editingHKey?<div style={{display:"flex",gap:6,marginTop:2}}><Btn size="sm" variant="accent" onClick={async()=>{const nk=s._newHKey||"";if(!nk)return;setSv(true);try{await api.post("/settings",{hermeece_api_key:nk});upd("_editingHKey",false);upd("_newHKey","");const fresh=await api.get("/settings");setS(fresh);setMsg("Key saved!")}catch{setMsg("Error")}setSv(false)}}>Save Key</Btn><Btn size="sm" variant="ghost" onClick={()=>upd("_editingHKey",false)}>Cancel</Btn></div>:null}</div>}</SF>
