...
{/* ── Unified scan progress ── One row per active or recently
     ...
     to call). */}
{scans.map(scan=>{const isLookup=scan.kind==="lookup";const isMam=scan.kind==="mam";const isLibrary=scan.kind==="library";const ex=scan.extra||{};const pctVal=scan.total>0?Math.round((scan.current/scan.total)*100):0;const cancelEndpoint=isLookup?"/lookup/cancel":scan.type==="full_scan"?"/mam/full-scan/cancel":"/mam/scan/cancel";return<div key={scan.kind} style={{marginTop:12,background:t.bg4,borderRadius:8,padding:"10px 14px"}}>{scan.running?<div>
...
</div>:<div style={{fontSize:13,color:scan.status==="complete"?t.grnt:t.redt}}>{scan.status==="complete"?<>{scan.label} Complete — {isLookup?`${scan.current} authors checked, ${ex.new_books||0} new books found`:isLibrary?`${scan.current} books synced — ${ex.books_new||0} new, ${ex.books_updated||0} updated`:(()=>{const rem=ex.remaining!=null?ex.remaining-(scan.current||0):(scan.total||0)-(scan.current||0);return`${scan.current} scanned: ${ex.found||0} found, ${ex.possible||0} possible, ${ex.not_found||0} not found${ex.errors>0?`, ${ex.errors} errors`:""}${rem>0?` · ${rem.toLocaleString()} unscanned`:""}`})()}</>:`${scan.label}: ${scan.status}`}</div>}</div>})}
