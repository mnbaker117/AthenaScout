...
{isLookup?<span style={{fontSize:11,color:t.tg}}>New books found: <b style={{color:t.grnt}}>{ex.new_books||0}</b></span>:isCalibre?<span style={{fontSize:11,color:t.tg}}>New: <b style={{color:t.grnt}}>{ex.books_new||0}</b> · Updated: <b style={{color:t.text2}}>{ex.books_updated||0}</b></span>:<div style={{display:"flex",gap:12,fontSize:11,color:t.tg}}><span style={{color:t.grnt}}>Found: {ex.found||0}</span><span style={{color:t.ylwt}}>Possible: {ex.possible||0}</span><span style={{color:t.redt}}>Not found: {ex.not_found||0}</span>{ex.errors>0?<span style={{color:t.red}}>Errors: {ex.errors}</span>:null}</div>}
{isCalibre?null:<Btn size="sm" onClick={async()=>{try{const r=await api.post(cancelEndpoint);toast.info(r.message||"Cancellation requested");window.dispatchEvent(new CustomEvent("athenascout:scan-started"))}catch(e){toast.error(e.message||"Cancel failed")}}} style={{background:t.red+"22",color:t.redt,border:`1px solid ${t.red}44`,padding:"2px 8px",fontSize:11}}>Stop</Btn>}</div>
</div>:<div style={{fontSize:13,color:scan.status==="complete"?t.grnt:t.redt}}>{scan.status==="complete"?<>{scan.label} Complete — {isLookup?`${scan.current} authors checked, ${ex.new_books||0} new books found`:isCalibre?`${scan.current} books synced — ${ex.books_new||0} new, ${ex.books_updated||0} updated`:(()=>{const rem=ex.remaining!=null?ex.remaining-(scan.current||0):(scan.total||0)-(scan.current||0);return`${scan.current} scanned: ${ex.found||0} found, ${ex.possible||0} possible, ${ex.not_found||0} not found${ex.errors>0?`, ${ex.errors} errors`:""}${rem>0?` · ${rem.toLocaleString()} unscanned`:""}`})()}</>:`${scan.label}: ${scan.status}`}</div>}</div>})}
