...
const[mamVld,setMamVld]=useState(false);const[mamRes,setMamRes]=useState(null);const[fsStatus,setFsStatus]=useState(null);const[dragIdx,setDragIdx]=useState(null);const[testRun,setTestRun]=useState(false);const[testRes,setTestRes]=useState(null);const[newSrcPath,setNewSrcPath]=useState("");const[newSrcType,setNewSrcType]=useState("root");const[pathVld,setPathVld]=useState(false);const[pathRes,setPathRes]=useState(null);useEffect(()=>{api.get("/settings").then(setS).catch(console.error)},[]);
...
<Btn size="sm" variant="accent" onClick={()=>{if(!newSrcPath.trim())return;const arr=[...(s.library_sources||[]),{path:newSrcPath.trim(),type:newSrcType}];upd("library_sources",arr);setNewSrcPath("");setPathRes(null)}} disabled={!newSrcPath.trim()}>Add</Btn>
</div>
