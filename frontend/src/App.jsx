...
// ─── Book Card ──────────────────────────────────────────────
function BCard({book,onAction,onClick,showAuthor,highlightAuthorId}){const t=T();const isUp=!!book.is_unreleased;const hasCover=book.cover_url||book.cover_path;const isOtherAuthor=highlightAuthorId&&book.author_id&&book.author_id!==highlightAuthorId;
...
<div style={{padding:"8px 10px"}}><div style={{fontSize:13,fontWeight:600,color:t.text2,lineHeight:1.3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{book.title}</div>{book.series_name&&book.series_index?<div style={{fontSize:10,color:t.purt,marginTop:1}}>#{book.series_index}{book.series_total?` of ${book.series_total}`:""}</div>:null}{showAuthor&&book.author_name?<div style={{fontSize:11,color:isOtherAuthor?t.ylwt:t.td,marginTop:2}}>{book.author_name}</div>:null}{isUp&&book.expected_date?<div style={{fontSize:11,color:t.cyant,marginTop:2}}>Expected: {fmtDate(book.expected_date)}</div>:null}</div></div>}

// ─── Book List Row ──────────────────────────────────────────
function BListRow({book,onAction,onClick,showAuthor,highlightAuthorId}){const t=T();const isOtherAuthor=highlightAuthorId&&book.author_id&&book.author_id!==highlightAuthorId;return<tr onClick={()=>onClick&&onClick(book)} style={{cursor:"pointer",borderBottom:`1px solid ${t.borderL}`,opacity:isOtherAuthor?0.55:1}}><td style={{padding:"8px 12px",fontSize:13,color:t.text2}}>{book.title}{book.is_new?<span style={{marginLeft:8,fontSize:9,fontWeight:700,background:t.red,color:"#fff",padding:"1px 5px",borderRadius:3}}>NEW</span>:null}{book.is_unreleased?<span style={{marginLeft:8,fontSize:9,fontWeight:700,background:t.cyan,color:"#fff",padding:"1px 5px",borderRadius:3}}>UPCOMING</span>:null}</td>{showAuthor?<td style={{padding:"8px 12px",fontSize:13,color:isOtherAuthor?t.ylwt:t.td}}>{book.author_name}</td>:null}<td style={{padding:"8px 12px",fontSize:13,color:t.td}}>{book.series_name?`${book.series_name}${book.series_index?` #${book.series_index}`:""}${book.series_total?` (${book.series_total})`:""}`:"—"}</td><td style={{padding:"8px 12px",fontSize:13,color:book.pub_date?t.td:book.expected_date?t.cyant:t.tg}}>{book.pub_date?fmtDate(book.pub_date):book.expected_date?fmtDate(book.expected_date):"Unknown"}</td><td style={{padding:"8px 12px",fontSize:11,color:t.tg}}>{book.source||"—"}</td></tr>}

// ─── Book Grid + List Wrappers ──────────────────────────────
function BGrid({books,onAction,onBookClick,showAuthor,highlightAuthorId}){return<div className="book-grid" style={{display:"flex",flexWrap:"wrap",gap:12,alignItems:"start"}}>{books.map(b=><BCard key={b.id} book={b} onAction={onAction} onClick={onBookClick} showAuthor={showAuthor} highlightAuthorId={highlightAuthorId}/>)}</div>}
function BList({books,onAction,onBookClick,showAuthor=false,highlightAuthorId}){const t=T();return<table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{borderBottom:`2px solid ${t.border}`}}><th style={{padding:"8px 12px",textAlign:"left",fontSize:11,fontWeight:600,color:t.tg,textTransform:"uppercase"}}>Title</th>{showAuthor?<th style={{padding:"8px 12px",textAlign:"left",fontSize:11,fontWeight:600,color:t.tg,textTransform:"uppercase"}}>Author</th>:null}<th style={{padding:"8px 12px",textAlign:"left",fontSize:11,fontWeight:600,color:t.tg,textTransform:"uppercase"}}>Series</th><th style={{padding:"8px 12px",textAlign:"left",fontSize:11,fontWeight:600,color:t.tg,textTransform:"uppercase"}}>Date</th><th style={{padding:"8px 12px",textAlign:"left",fontSize:11,fontWeight:600,color:t.tg,textTransform:"uppercase"}}>Source</th></tr></thead><tbody>{books.map(b=><BListRow key={b.id} book={b} onAction={onAction} onClick={onBookClick} showAuthor={showAuthor} highlightAuthorId={highlightAuthorId}/>)}</tbody></table>}
...
<div style={{fontSize:12,fontWeight:600,color:t.tm,textTransform:"uppercase",letterSpacing:"0.06em",padding:"14px 0 6px"}}>API Keys</div>
<SF label="Hardcover API Key" desc="Get from hardcover.app → Account → API">{s.hardcover_api_key_set&&!s._editingKey?<div style={{display:"flex",alignItems:"center",gap:12}}><span style={{fontSize:14,color:t.tm,letterSpacing:"3px"}}>••••••••</span><Btn size="sm" onClick={()=>upd("_editingKey",true)}>Change</Btn></div>:<div style={{display:"flex",flexDirection:"column",gap:6}}><input value={s._editingKey?s._newKey||"":s.hardcover_api_key||""} onChange={e=>s._editingKey?upd("_newKey",e.target.value):upd("hardcover_api_key",e.target.value)} placeholder="Bearer eyJ..." style={{...ist,width:260}}/>{s._editingKey?<div style={{display:"flex",gap:6,marginTop:2}}><Btn size="sm" variant="accent" onClick={async()=>{const nk=s._newKey||"";if(!nk)return;setSv(true);try{await api.post("/settings",{hardcover_api_key:nk});upd("_editingKey",false);upd("_newKey","");const fresh=await api.get("/settings");setS(fresh);setMsg("Key saved!")}catch{setMsg("Error")}setSv(false)}}>Save Key</Btn><Btn size="sm" variant="ghost" onClick={()=>upd("_editingKey",false)}>Cancel</Btn></div>:null}</div>}</SF>
</div>
...
{/* Sources */}
<div style={{background:t.bg2,border:`1px solid ${t.border}`,borderRadius:12,padding:"4px 20px"}}>
<div style={{fontSize:12,fontWeight:600,color:t.tm,textTransform:"uppercase",letterSpacing:"0.06em",padding:"14px 0 6px"}}>Sources (in priority order)</div>
...
{/* Languages */}
<div style={{background:t.bg2,border:`1px solid ${t.border}`,borderRadius:12,padding:"4px 20px"}}>
<div style={{fontSize:12,fontWeight:600,color:t.tm,textTransform:"uppercase",letterSpacing:"0.06em",padding:"14px 0 6px"}}>Languages</div>
...
{/* ── Below only visible when MAM is enabled ── */}
{s.mam_enabled?<>
...
{/* Book list */}
{ld?<Load/>:books.length===0?<div style={{textAlign:"center",padding:40,color:t.tg}}>No books in this section</div>:vm==="list"?<BList books={books} onAction={onAction} onBookClick={toggleSb}/>:<BGrid books={books} onAction={onAction} onBookClick={toggleSb}/>}
