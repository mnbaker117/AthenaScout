...
_lookup_progress: Dict[str, Any] = {
    ...
    "total": 0,
    "current_author": "",
    # Per-book progress for source scans. Set by Goodreads/Kobo/Hardcover
    # via the `_on_book` closure that lookup.py stashes on each source
    # instance. Only emitted for books that actually consume work (DETAIL
    # page fetches + URL-backfill matches), so the user-visible feed
    # never flickers through filter noise (foreign / set / translation /
    # contributor-only / unowned skips). Cleared between authors so a
    # stale title from one author doesn't bleed into the next.
    "current_book": "",
    "new_books": 0,
    "status": "idle",
    "type": "none",
...
_mam_scan_progress: Dict[str, Any] = {
    ...
    "not_found": 0,
    "errors": 0,
    # Title MAM is currently checking. Updated per-book by
    # scan_books_batch (via the on_progress stats dict) and by the
    # single-author scan loop in routers/mam.py. MAM shows EVERY
    # attempt — there's no filter-noise to hide here.
    "current_book": "",
    "status": "idle",
    "type": "none",
}
